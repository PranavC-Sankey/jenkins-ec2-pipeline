pipeline {
    ...
    stages {
        ...
        stage('Deploy to EC2') {
            steps {
                bat '''
                "%GIT_BASH%" -c "chmod 400 '%PEM_KEY_PATH%' && echo '📦 Backup current deployment...' && ssh -o StrictHostKeyChecking=no -i '%PEM_KEY_PATH%' ${env.EC2_USER}@${env.EC2_HOST} 'sudo mkdir -p /tmp/rollback-${params.TARGET_ENVIRONMENT} && sudo rm -rf /tmp/rollback-${params.TARGET_ENVIRONMENT}/* && sudo cp -r ${env.NGINX_ROOT_DIR}/* /tmp/rollback-${params.TARGET_ENVIRONMENT}/' && echo '📤 Uploading build...' && scp -o StrictHostKeyChecking=no -i '%PEM_KEY_PATH%' -r dist/* ${env.EC2_USER}@${env.EC2_HOST}:${env.REMOTE_DEPLOY_DIR}/ && echo '⚙️ Deploying new build...' && ssh -o StrictHostKeyChecking=no -i '%PEM_KEY_PATH%' ${env.EC2_USER}@${env.EC2_HOST} 'sudo rm -rf ${env.NGINX_ROOT_DIR}/* && sudo cp -r ${env.REMOTE_DEPLOY_DIR}/* ${env.NGINX_ROOT_DIR}/ && echo Deployed ${params.VERSION} to ${params.TARGET_ENVIRONMENT} on \$(date) | sudo tee ${env.NGINX_ROOT_DIR}/VERSION.txt && sudo systemctl restart nginx'"
                '''
            }
        }
    }
